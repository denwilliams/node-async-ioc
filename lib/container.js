...
var IGNORE_FILES = ['.DS_Store','README.md','readme.md'];
...
function Container() {
  ...
  this._recurseServices = [];
  this._recurseInterfaces = [];
  this._interfaces = {};
  this._stopSequence = [];
  this._isDebug = false;
...
/**
 * Recurses folders matching the following name for additional services
 * 
 * @param {[String]} folderNames
 */
Container.prototype.recurseServices = function(folderNames) {
  this._recurseServices = folderNames;
  return this;
};

/**
 * Recurses folders matching the following name for additional interfaces
 * 
 * @param {[String]} folderNames
 */
Container.prototype.recurseInterfaces = function(folderNames) {
  this._recurseInterfaces = folderNames;
  return this;
};
 ...
 * @param  {String|Object} interfaceName - Either the name of the interface, or an interface definition
 * @param  {[String]} methods - Array of string method names allowed/required on the interface
 * @param  {[String]} properties - Array of string property names allowed/required on the interface
...
/**
 * Defines all interfaces in
 * @param  {[type]} path [description]
 * @return {[type]}      [description]
 */
Container.prototype.defineAll = Container.prototype.defineFolder = function(path) {
  var self = this;

  var files = fs.readdirSync(path);

  files.forEach(function(file) {
    if (IGNORE_FILES.indexOf(file) >= 0) return;

    if (self._recurseServices.indexOf(file) >= 0) {
      self.registerFolder(path + '/' + file);
      return;
    }
    if (self._recurseInterfaces.indexOf(file) >= 0) {
      self.defineFolder(path + '/' + file);
      return;
    }
...
Container.prototype.register = function(serviceNames, factory, dependencies, options) {
  ...
  self._debug('Registering service "' + name + '" with ' + dependencies.length + ' dependencies');
 ...
 */
Container.prototype.registerAll = Container.prototype.registerFolder = function(path) {
  var self = this;
...
Container.prototype.start = Container.prototype.get = function(serviceName) {
  ...
  return Q.try(function() {
    ...
    return registration.call()
      .fail(function(err) {
        self._log(err.stack);
        throw new Error('Could not start ' + serviceName + ': ' + err.message);
      });
  });
};
...
Container.prototype._getDependencies = function(dependencies) {
  ...
  dependencies.forEach(function(serviceName) {
    ...
    promises.push(
      ...
      // .fail(function(err) {
      //   self._log(err.stack);
      //   throw new Error('Dependency "' + serviceName + '" failed to load: ' + err.message);
      // })
    );
  });
